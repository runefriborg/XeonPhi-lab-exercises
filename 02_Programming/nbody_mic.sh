...
set -x
micnativeloadex ./nbody.mic -a ${NBODIES} -e "KMP_AFFINITY=compact,granularity=fine${VERBOSE}" -d 0

# alternative command if NFS is working for $PWD and $MIC_LD_LIBRARY_PATH (usually /opt/intel/...)
#ssh mic0 "cd $PWD; LD_LIBRARY_PATH=$MIC_LD_LIBRARY_PATH KMP_AFFINITY=compact,granularity=fine${VERBOSE} ./nbody ${NBODIES}"
