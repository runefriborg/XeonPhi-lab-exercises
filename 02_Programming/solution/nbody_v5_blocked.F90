...
subroutine Perform_NBody()
  ...
  !$omp parallel private(i,j,jj,pos_x,pos_y,pos_z,acc_x,acc_y,acc_z,delta_x,delta_y,delta_z,gamma,s)
  do jj = 1, number_of_bodies, CHUNK_SIZE
    !$omp do
    do i = 1, number_of_bodies
      pos_x = Position_X(i)
      pos_y = Position_Y(i)
      ...
      !$omp simd private(delta_x,delta_y,delta_z,gamma,s) reduction(+:acc_x, acc_y, acc_z) aligned(Position_X, Position_Y, Position_Z, Mass :64)
      do j = jj, min(jj + CHUNK_SIZE, number_of_bodies)
